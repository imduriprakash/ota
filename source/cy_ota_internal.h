 ...
 */
#define OTA_HTTP_SIZE_OF_RECV_BUFFER        (4 * 1024)
...
typedef enum
{
    ...
    OTA_EVENT_GOT_DATA                      = (1 << 15),    /**< Notify cy_ota_TRANSPORT_get() we got some data */
    OTA_EVENT_DATA_DONE                     = (1 << 16),    /**< Notify cy_ota_get() that we got all the data   */
    OTA_EVENT_DATA_FAIL                     = (1 << 17),    /**< Data failure (could be sign / decrypt fail)    */
    OTA_EVENT_REDIRECT                      = (1 << 18),    /**< Re-direct to for OTA download                  */
    OTA_EVENT_DROPPED_US                    = (1 << 19),    /**< MQTT Broker / HTTP Server dropped us           */
...
/* OTA MQTT main loop events to wait look for */
#define OTA_EVENT_HTTP_EVENTS  ( OTA_EVENT_AGENT_SHUTDOWN_NOW | \
                                    OTA_EVENT_AGENT_PACKET_TIMEOUT | \
                                    OTA_EVENT_AGENT_DOWNLOAD_TIMEOUT | \
                                    OTA_EVENT_AGENT_STORAGE_ERROR | \
                                    OTA_EVENT_AGENT_DISCONNECT | \
...
typedef struct cy_ota_http_context_s {
    IotNetworkConnection_t  connection;
    ...
    uint8_t     data_buffer[OTA_HTTP_SIZE_OF_RECV_BUFFER];  /**< Used to get HTTP data                  */
...
typedef struct cy_ota_context_s {
    ...
    cy_ota_server_info_t    curr_server;            /**< current server information (may have changed by a redirect)    */
    union {
    ...
    cy_ota_server_info_t    new_server;             /**< Server to switch to                                            */
} cy_ota_context_t;
